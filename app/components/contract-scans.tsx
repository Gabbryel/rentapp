    ...
    <div className="mb-8 rounded-md border border-sky-300/40 bg-sky-50/60 dark:bg-sky-900/20 p-4 text-[#F5F1DC]">
      ...
      <div className="grid grid-cols-1 gap-2">
        {scans.map((s, idx) => (
          <div
            key={`${s.url}-${idx}`}
            className="rounded-md border border-foreground/10 bg-background/60"
          >
            ...
            {/* Inline Edit Form */}
            {editingIdx === idx && editScanAction && contractId && (
              <div className="border-t border-foreground/10 px-3 py-3 bg-background/40">
                <form
                  onSubmit={async (e) => {
                    e.preventDefault();
                    if (editScanAction && contractId) {
      ...
      >
        <form
          onSubmit={async (e) => {
            e.preventDefault();
            if (deleteScanAction && contractId) {
              const formData = new FormData(e.currentTarget);
