...
export default function ManageContractScans({
  ...
  return (
    <div className="mt-4 rounded-md border border-foreground/15 p-4 bg-[#34656D] text-[#FAEAB1]">
      <h3 className="text-sm font-semibold mb-3">Gestionează documentele</h3>
      {msg ? (
        <div className="mb-2 rounded border border-amber-500/30 bg-amber-500/10 px-2 py-1 text-xs text-amber-800">
          {msg}
        </div>
      ) : null}
              ...
              >
                ...
                <div className="shrink-0 flex items-center gap-2">
                  ...
                  <form
                    action={removeAction}
                    onSubmit={(e) => {
                      if (!confirm("Ștergi acest scan?")) e.preventDefault();
                    }}
                  >
                    <input type="hidden" name="id" value={id} />
                    <input type="hidden" name="index" value={idx} />
